...
Multi-Agent Slack Bot system where multiple Slack bots converse with each other. Each bot connects via Socket Mode, responds when @mentioned, calls Claude via the `claude` CLI (print mode with hooks disabled for speed), with a persona defined in a SOUL.md file, and posts the response directly to the Slack channel. Bots can @mention other bots, creating chain reactions.
...
**Key flow**: Slack @mention → `AgentBot._handle_message` → resolve mentions to names → append to per-thread conversation history → build system prompt (SOUL.md + team context) → async `call_claude()` via Claude CLI subprocess → post response directly to Slack channel.
...
- `call_claude()` — async function that spawns `claude -p` subprocess with `--output-format json` and hooks disabled for speed (~6s vs ~50s); formats conversation history into prompt with system context
...
- **Claude CLI subprocess**: Uses `claude -p --output-format json` with `--settings '{"hooks":{}}' --setting-sources '' --disable-slash-commands` to disable hooks/plugins for speed (~6s total, ~2s API). Conversation history is formatted into the prompt string. OAuth auth is inherited from the CLI's existing session.
